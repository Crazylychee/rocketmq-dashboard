...
var app = angular.module('app', [
    ...
    return apiConfig;
        ...
        ['$rootScope','$location','$cookies','$http', '$window','Notification', 'appConfig',
            function ($rootScope,$location,$cookies,$http, $window, Notification, appConfig) {
                // Make config available in rootScope
                ...
                var init = function(callback){
                    ...
                    var url = appConfig.apiBaseUrl + '/login/check.query';
                    var setting = {
                                type: "GET",
                                timeout:15000,
                                success:callback,
                ...
                $http({
                    method: "GET",
                    url: appConfig.apiBaseUrl + "/acl/enable.query"
...
// HTTP拦截器
app.factory('httpInterceptor', ['$q', '$injector', 'appConfig', function($q, $injector, appConfig) {
    return {
        request: function(config) {
            // 检查URL是否已经是绝对URL（以http://或https://开头）
            if (config.url && !config.url.match(/^(http|https):\/\//)) {
                // 检查URL是否已经包含API基础URL
                if (config.url.indexOf(appConfig.apiBaseUrl) !== 0) {
